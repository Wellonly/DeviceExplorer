...
quint32 QHidApiPrivate::getDeviceId(int fd)
{
    ...
    return getDeviceId(stat_info.st_dev);
}

quint32 QHidApiPrivate::getDeviceId(__dev_t stdev)
{
    quint32 ret = mDeviceNumber2DeviceId_map.value(stdev, 0);
    if (!ret) {
        ret = open(stdev);
        mDeviceNumber2DeviceId_map[stdev] = ret;
    }
    return ret;
}
/// *** zvv...
...
/*!
 * \brief Open a HID device using a file descriptor upon the device.
 *
 * Opens a HID device with the supplied file descriptor. It find serial_number,
 * VID and PID of the device which file descriptor belongs. The method returns an id number
 * which should be retained as it is used to identify the device that you wish to access.
 *
 * \param fd The file descriptor of a file on the device.
 * \return returns an quint32 id number, or 0 if unsuccessful.
 */
quint32 QHidApiPrivate::open(__dev_t stdev)
{
  quint32 ret = 0;
  udev_hid_attributes hid_attributes;
  if (get_udev_hid_attributes(stdev, &hid_attributes)) {
      ushort vendorId, productId;
      int res = sscanf(hid_attributes.idVendor,  "%hx", &vendorId);
      res +=    sscanf(hid_attributes.idProduct, "%hx", &productId);
      if (res == 2) {
        ret = open(vendorId, productId, QString(hid_attributes.serial));
      }
      free(hid_attributes.idVendor);
      free(hid_attributes.idProduct);
      free(hid_attributes.serial);
      free(hid_attributes.manufacturer);
  }
  return ret;
}
...
QByteArray QHidApiPrivate::read(quint32 id, int timeout) {
    ...
    if (device != NULL) {
        ...
        int rep = hid_read_timeout(device, buf, 256, timeout);
...
//        device = hid_open(0x4d8, 0xf1fa, NULL);
        device = hid_open(vendorId, productId, sn);
        free(sn);
    }
    if (device != NULL) {
      id = nextId();
...
bool QHidApiPrivate::get_udev_hid_attributes(__dev_t stdev, udev_hid_attributes *attrs)
{
    struct udev *udev;
    ...
    if (!udev) { //debug_msg("Can't create udev\n");
        return false;
    }

    /* Open a udev device from the dev_t. 'b' or 'c' means block or character device. */
    udev_dev = udev_device_new_from_devnum(udev, 'b' /*'c'*/, stdev /*s.st_rdev*/);
    if (udev_dev) {
        hid_dev = udev_dev;
        while (hid_dev != NULL) {
            const char* manufacturer = udev_device_get_sysattr_value(hid_dev, "manufacturer");
            if (manufacturer) {
                attrs->manufacturer = strdup(manufacturer);
                attrs->idVendor  = strdup(udev_device_get_sysattr_value(hid_dev, "idVendor"));
                attrs->idProduct = strdup(udev_device_get_sysattr_value(hid_dev, "idProduct"));
                attrs->serial    = strdup(udev_device_get_sysattr_value(hid_dev, "serial"));
                ret = true;
                break;
            }
//            debug_msg("udev_device_get_parent:" << udev_device_get_sysname(hid_dev));
//            debug_msg("udev_device_get_sysattr_value(idVendor):" << udev_device_get_sysattr_value(hid_dev,"idVendor"));
//            debug_msg("udev_device_get_sysattr_value(idProduct):" << udev_device_get_sysattr_value(hid_dev,"idProduct"));
//            debug_msg("udev_device_get_sysattr_value(serial):" << udev_device_get_sysattr_value(hid_dev,"serial"));
//            debug_msg("udev_device_get_sysattr_value(manufacturer):" << udev_device_get_sysattr_value(hid_dev,"manufacturer"));
