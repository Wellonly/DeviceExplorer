...
QHidApi::QHidApi(const QHidApi &other)
...
quint32 QHidApi::getDeviceId(int fd)
{
    return d_ptr->getDeviceId(fd);
}

quint32 QHidApi::getDeviceId(__dev_t stdev)
{
    return d_ptr->getDeviceId(stdev);
}

/*!
 * \brief Open a HID device using a device number from stat info.
 *
 ...
 *
 * \param stdev The device number from stat info.
 * \return returns an quint32 id number, or 0 if unsuccessful.
 */
quint32 QHidApi::open(__dev_t stdev)
{
    return d_ptr->open(stdev);
}
